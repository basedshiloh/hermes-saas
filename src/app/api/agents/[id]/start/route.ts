...
import { startContainer } from "@/src/lib/worker-client";
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await db.agentInstance.update({ where: { id: agent.id }, data: { state: "RUNNING" } });
    return NextResponse.json({ success: true });
  } catch {
    return NextResponse.json({ error: "Failed to start" }, { status: 502 });
  }
}
